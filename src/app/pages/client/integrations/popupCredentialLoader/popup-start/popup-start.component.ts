...
@Component({
  selector: 'app-popup-start',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './popup-start.component.html',
  styleUrl: './popup-start.component.scss'
})
          ...
          await this.web3WalletService.getEthAddresses().then((accounts) => {
            if (accounts && accounts?.length > 0) {
              ...
              this.historyService.hasHistoryKey().subscribe({
                next: (response) => {
                  if (response.hasKey) {
                    this.router.navigateByUrl('popup/review', { state: { referrer: this.referrer } });
                  } else {
